...
public class ListDialogAdapter33 extends RecyclerView.Adapter<ListDialogAdapter33.ViewHolder> implements TreeStateChangeListener {
    private final static int ITEM_STATE_CLOSE = 0;
    private final static int ITEM_STATE_OPEN = 1;
    private Context mContext;
    private List<TreeItem> mList;

    public ListDialogAdapter33(Context context, List<TreeItem> list) {
        initList(list, 0);
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view;
        view = LayoutInflater.from(mContext).inflate(R.layout.dialog_list_item33, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(final ListDialogAdapter33.ViewHolder viewHolder, int i) {
        ...
        if (treeItem.getId() == selectDeptDd){
            viewHolder.ll_parent.setBackgroundColor(ContextCompat.getColor(mContext, R.color.color_ffe1e2));
        }
