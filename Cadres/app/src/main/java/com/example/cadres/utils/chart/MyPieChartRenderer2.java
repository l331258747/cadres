...
public class MyPieChartRenderer2 extends DataRenderer {
    ...
    public MyPieChartRenderer2(PieChart chart, ChartAnimator animator,
        ...
        mHolePaint.setColor(Color.TRANSPARENT);
        mHolePaint.setStyle(Style.FILL);

        //内部透明圆与内部圆间距(31f-28f)填充颜色
        mTransparentCirclePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mTransparentCirclePaint.setColor(Color.TRANSPARENT);
...
//                    + Utils.convertDpToPixel(4f);
            float lineHeight = 0;
            float lineWight = Utils.calcTextWidth(mValuePaint, "10.0%") + Utils.convertDpToPixel(4f);
            ...
            for (int j = 0; j < entryCount; j++) {
                ...
                lineWight = Utils.calcTextWidth(mValuePaint, formattedValue) + Utils.convertDpToPixel(4f);
...
//                        pt2x = pt1x + polyline2Width;
                    ...
                    if (dataSet.getValueLineColor() != ColorTemplate.COLOR_NONE && value > 0 && entryLabel != null) {
                    ...
                    if (drawXOutside && drawYOutside) {
                        ...
                        if (j < data.getEntryCount() && entryLabel != null && value > 0) {
                            if (transformedAngle % 360.0 >= 90.0 && transformedAngle % 360.0 <= 270.0) {//左边部分
                                drawEntryLabel(c, entryLabel, labelPtx - lineWight, labelPty + lineHeight);
                        ...
                        if (j < data.getEntryCount() && entryLabel != null) {
                            drawEntryLabel(c, entryLabel, labelPtx + lineWight, labelPty + lineHeight / 2.f);
                        }
                    } else if (drawYOutside) {

                        drawValue(c, formattedValue, labelPtx + lineWight, labelPty + lineHeight / 2.f, dataSet.getValueTextColor(j));
                    }
                }
                ...
                if (drawXInside || drawYInside) {
                    ...
                    if (drawXInside && drawYInside) {
                        ...
                        if (j < data.getEntryCount() && entryLabel != null) {
                            drawEntryLabel(c, entryLabel, x + lineWight, y + lineHeight);
                        }

                    } else if (drawXInside) {
                        if (j < data.getEntryCount() && entryLabel != null) {
                            drawEntryLabel(c, entryLabel, x + lineWight, y + lineHeight / 2f);
                        }
                    } else if (drawYInside) {
                        drawValue(c, formattedValue, x + lineWight, y + lineHeight / 2f, dataSet.getValueTextColor(j));
                    }
                }
